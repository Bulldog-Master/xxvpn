-- Enforce rate limiting and add sanitization
...
-- 3. Add validation to limit context size (prevent DoS)
ALTER TABLE public.error_logs
  ADD CONSTRAINT error_context_size_limit 
  CHECK (pg_column_size(context) < 50000); -- Max 50KB
...
COMMENT ON TRIGGER sanitize_error_log_context ON public.error_logs IS
  'SECURITY: Automatically sanitizes error context before insertion.
   ...
   Also enforces 50KB size limit to prevent DoS attacks.';